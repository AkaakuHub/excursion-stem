        ...
        try:
            ...
            for instrument in instrument_dirs:
                ...
                if matching_files:
                    ...
                    alt_files = [f for f in glob.glob(alt_pattern) if instrument in os.path.basename(f).lower()]
                    if alt_files:
                        tracks[instrument] = alt_files[0]
                        logger.info(f"{instrument}パートを代替パターンで発見: {alt_files[0]}")
            
            if not tracks:
                raise FileNotFoundError(f"分離されたパートが見つかりません: {base_path}")
